...
- **Library** (`VanityNumberApi.DictionarySanitizer`): Reusable class library for dictionary processing
- **Console Tool** (`VanityNumberApi.DictionarySanitizer.Tool`): Command-line application for easy dictionary generation
...
```bash
# Run with default path (../VanityNumberApi.Core/Dictionaries)
dotnet run --project VanityNumberApi.DictionarySanitizer.Tool

# Run with custom path
dotnet run --project VanityNumberApi.DictionarySanitizer.Tool -- "C:\path\to\dictionaries"
```
...
```bash
# Build library
dotnet build VanityNumberApi.DictionarySanitizer

# Build console tool
dotnet build VanityNumberApi.DictionarySanitizer.Tool
...
- Both files ready to use with VanityNumberApi.Core
